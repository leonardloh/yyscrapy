...
class YueYuSpider(scrapy.Spider):
    ...
    def parse(self, response):
        img_width = 150 #one of [150, 220, 360, 470, 600, 750, 940, 1080, 1296, 1512, 1728, 2048]
        for products in response.css('div.wide--one-fifth'):
            ...
            yield {
                'collection': response.css("h1.section-header__title::text").get(),
                'name': products.css('p.grid-link__title::text').get(),
                'price': next(filtered_empty_scrapped_prices),
                'img': "https:" + response.css('img.product__img').attrib['data-src'].replace('{width}', f'{img_width}')
            }
